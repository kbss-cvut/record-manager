...
class RecordFilterMapperTest {
    ...
    static Stream<Arguments> testValues() {
        return Stream.of(
                ...
                )), new RecordFilterParams(null, Set.of(), LocalDate.now().minusYears(1), LocalDate.now(),
                                           Collections.emptySet(), Collections.emptySet())),
                Arguments.of(new LinkedMultiValueMap<>(Map.of(
                        "minDate", List.of(LocalDate.now().minusYears(1).toString()),
                        "maxDate", List.of(LocalDate.now().minusDays(1).toString())
                )), new RecordFilterParams(null, Set.of(), LocalDate.now().minusYears(1), LocalDate.now().minusDays(1),
                                           Collections.emptySet(), Collections.emptySet())),
                Arguments.of(new LinkedMultiValueMap<>(Map.of(
                        "institution", List.of("1111111")
                )), new RecordFilterParams(null,Set.of("1111111"), LocalDate.EPOCH, LocalDate.now(), Collections.emptySet(), Collections.emptySet())),
                Arguments.of(new LinkedMultiValueMap<>(Map.of(
                        "institution", List.of("1111111"),
                        "phase", List.of(RecordPhase.open.getIri(), RecordPhase.completed.name())
                )), new RecordFilterParams(null, Set.of("1111111"), LocalDate.EPOCH, LocalDate.now(),
                ...
                )), new RecordFilterParams(null,Set.of("1111111"), LocalDate.now().minusYears(1), LocalDate.now().minusDays(1),
